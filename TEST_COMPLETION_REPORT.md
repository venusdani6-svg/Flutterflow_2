...
| 4 | Bulk-invite ≤5 favorites, `group_invite` auto-hide | **TRACED-PARTIAL** | Bulk-send chain fully correct (cap enforced, multi-cast reservation created correctly). The auto-hide requirement is explicitly NOT wired — confirmed as a disclosed, deliberate scope cut in the DSL's own comment, not an oversight. |
...
Several scenarios surfaced gaps that were already disclosed elsewhere in this project's own documentation before this pass — re-confirmed as still-true, not newly discovered: search/filter (§3.3, `[PARTIAL]`), affiliate-network-pull as an alternate group-invite mechanism (`[MISSING]`), bulk-invite's `group_invite` auto-hide (a deliberate, disclosed scope cut), admin work-post hire-UI (a deliberate scope decision). None of these were silently re-discovered and left unmentioned — each is cited by name above.
