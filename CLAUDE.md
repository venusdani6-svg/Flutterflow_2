...
5. If any of these files' content changes or new project knowledge is surfaced, update `PROJECT_KNOWLEDGE.md` (its own change-log convention) and `.cursor/rules/project_rules.md` afterward — and `PROJECT_ANALYSIS.md`/`IMPLEMENTATION_PLAN.md` too if the task changed something they document as current/planned state — and briefly report what was updated and why. Do not let this workspace's knowledge drift apart silently across sessions or across the tools that read it (`AGENTS.md` mirrors this same section for Kiro/Codex — keep both in sync).
