...
> **⚠️ STALE — flagged 2026-08-14, not refreshed.** This document is a snapshot generated 2026-08-09, before Phases 1-13 of `IMPLEMENTATION_PLAN.md` (auth/onboarding, reservation core, Stripe Connect payments, chat, reviews/tips/safety, wallet/payout, affiliate, work/staff board, My Page IA, and the full admin panel) were built. Its central conclusions — **"no real backend, Cloud Functions, or Stripe integration exists," most transactional buttons are unwired stubs, 24-25 mostly-static pages** — are no longer true: a ~11,000-line Cloud Functions backend (`firebase/functions/src/*.ts`) is built and largely deployed, the reservation/payment/chat/affiliate/admin systems are functionally wired end-to-end, and the app now has 70+ pages. Treat this file as a **historical record of the pre-build starting state only** — for current state, read `PROJECT_KNOWLEDGE.md` (the running change log, §1-§91+) and `IMPLEMENTATION_PLAN.md` (the requirements catalog, whose `[DONE]`/`[PARTIAL]`/`[MISSING]` tags are kept current). A handful of items below remain accurate simply because they were never in scope to fix (e.g. the Android package-path mismatch, §8 item 11) — but do not trust any claim in this document about backend/wiring completeness without cross-checking the two files above first. A full line-by-line refresh of this document was assessed as out of scope for the review pass that added this notice; flagging its staleness clearly was judged higher-value than a partial/rushed rewrite.
